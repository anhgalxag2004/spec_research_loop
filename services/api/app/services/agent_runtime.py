...
JUDGE_FOCUSES = {
    "Research Gap Judge": "Check whether the claimed research gap follows from cited or recorded limitations. Flag a gap that is only asserted because no identical paper was found.",
    "Contribution Judge": "Check whether contributions are clear, falsifiable, and not overstated beyond the stated evidence and scope.",
    "Experiment Judge": "Check whether baselines, metrics, controls, ablations, and held-out evaluation can test each claim fairly.",
    "Evidence Judge": "Check whether each claim is paired with evidence, a source, a reviewed passage, and a verdict. Do not infer external verification.",
    "Conference Readiness Judge": "Check originality, significance, soundness, clarity, and reproducibility from the supplied specification only.",
}
...
Do not invent publication URLs or claim that a source proves something you did not retrieve. For each claim_evidence item, set verification to INSUFFICIENT unless the user supplied verifiable evidence. Write a testable, falsifiable research plan in English. Cards need type, content, status. Related work needs title, year, url, approach, limitation. Claim evidence needs claim, baseline, metric, evidence_needed, falsification, verification, verification_rationale. Experiment items need experiment, purpose, setup.""",
        ...
        max_tokens=1400,
        timeout_seconds=settings.llm_interactive_timeout_seconds,
    )
    try:
...
def generate_judges(draft_spec: str) -> list[JudgeFeedback] | None:
    def generate_one(judge_name: str, focus: str) -> JudgeFeedback | None:
        response = _chat_json(
            f"""You are the {judge_name} in an independent research-spec review.
{focus}
You receive only the supplied specification, not any other Judge feedback. Return one JSON object with judge, severity (MINOR or MAJOR), issue, rationale, and recommendation. Do not claim external verification.""",
            draft_spec,
            max_tokens=420,
            timeout_seconds=settings.llm_interactive_timeout_seconds,
        )
        try:
            feedback = JudgeFeedback.model_validate(response)
    ...
    with ThreadPoolExecutor(
        max_workers=min(settings.llm_judge_parallelism, len(JUDGE_FOCUSES)),
    ) as executor:
        futures = {
            judge_name: executor.submit(generate_one, judge_name, focus)
            for judge_name, focus in JUDGE_FOCUSES.items()
        }
        results = {
            judge_name: future.result()
            for judge_name, future in futures.items()
        }
    if any(result is None for result in results.values()):
        return None
    return [results[judge_name] for judge_name in JUDGE_FOCUSES]
