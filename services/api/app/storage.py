...
def create_project(idea: str, draft_spec: str, readiness_score: int) -> tuple[str, str]:
    project_id = uuid4().hex[:12]
    created_at = datetime.now(UTC).isoformat()
    with _connection() as connection:
        connection.execute(
            "INSERT INTO projects (id, idea, created_at, current_version) VALUES (?, ?, ?, ?)",
            (project_id, idea, created_at, 1),
        )
        connection.execute(
...
def save_revision(project_id: str, draft_spec: str, change_log: list[str], readiness_score: int) -> int:
    ...
    with _connection() as connection:
        ...
        connection.execute("UPDATE projects SET current_version = ? WHERE id = ?", (version, project_id))
    return version
...
def get_latest_spec(project_id: str) -> dict[str, object]:
    with _connection() as connection:
        row = connection.execute(
            """SELECT spec_versions.version, draft_spec, projects.idea
    ...
    if row is None:
        raise ValueError("Project does not exist.")
    return {"version": row["version"], "draft_spec": row["draft_spec"], "idea": row["idea"]}
...
def get_judge_consensus(project_id: str) -> dict[str, object]:
    ...
    issues = [str(judge["issue"]) for judge in judges]
    return {
        "project_id": project_id,
        "spec_version_used": version,
        "judge_count": len(judges),
        "major_count": major_count,
        "minor_count": len(judges) - major_count,
